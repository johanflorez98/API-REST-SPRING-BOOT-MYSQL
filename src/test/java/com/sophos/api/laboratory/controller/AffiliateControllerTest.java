...
class AffiliateControllerTest {
    ...
    void getlist200() {
        ...
        when(affiliateService.getlist()).thenReturn(affiliate);
    ...
    @Test
    void getbyid200() {
        when(affiliateService.getbyid(anyLong())).thenReturn(Optional.of(new Affiliate()));
        assertEquals(HttpStatus.OK, affiliateController.getbyid(anyLong()).getStatusCode());
    }

    @Test
    void getbyid404() {
        when(affiliateService.getbyid(anyLong())).thenReturn(Optional.empty());
    ...
    void post201() {
        ...
        when(affiliateService.post(affiliate)).thenReturn("Save");
    ...
    void post404() {
        Affiliate affiliate = new Affiliate();
        when(affiliateService.post(affiliate)).thenReturn("No save");
        assertEquals(HttpStatus.NOT_FOUND, affiliateController.post(affiliate).getStatusCode());
    }

    @Test
    void delete200() {
        when(affiliateService.delete(anyLong())).thenReturn("deleted");
        assertEquals(HttpStatus.OK, affiliateController.delete(anyLong()).getStatusCode());
    }

    @Test
    void delete204() {
        when(affiliateService.delete(anyLong())).thenReturn("no deleted");
    ...
    void put201() {
        Affiliate affiliate = new Affiliate();
        when(affiliateService.put(affiliate)).thenReturn(Optional.of(affiliate));
    ...
    void put404() {
        Affiliate affiliate = new Affiliate();
        when(affiliateService.put(affiliate)).thenReturn(Optional.empty());
