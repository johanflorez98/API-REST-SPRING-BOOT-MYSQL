...
class TestControllerTest {
    ...
    void getlist200() {
        ...
        test.add(new Test());
        when(testServiceImp.getlist()).thenReturn(test);
    ...
    @org.junit.jupiter.api.Test
    void getbyid200() {
        when(testServiceImp.getbyid(anyLong())).thenReturn(Optional.of(new Test()));
        assertEquals(HttpStatus.OK, testController.getbyid(anyLong()).getStatusCode());
    }

    @org.junit.jupiter.api.Test
    void getbyid404() {
        when(testServiceImp.getbyid(anyLong())).thenReturn(Optional.empty());
    ...
    void post201() {
        Test test = new Test();
        when(testServiceImp.post(test)).thenReturn("Save");
    ...
    void post404() {
        Test test = new Test();
        when(testServiceImp.post(test)).thenReturn("No save");
        assertEquals(HttpStatus.NOT_FOUND, testController.post(test).getStatusCode());
    }

    @org.junit.jupiter.api.Test
    void delete200() {
        when(testServiceImp.delete(anyLong())).thenReturn("deleted");
        assertEquals(HttpStatus.OK, testController.delete(anyLong()).getStatusCode());
    }

    @org.junit.jupiter.api.Test
    void delete204() {
        when(testServiceImp.delete(anyLong())).thenReturn("no deleted");
    ...
    void put201() {
        Test test = new Test();
        when(testServiceImp.put(test)).thenReturn(Optional.of(test));
    ...
    void put404() {
        Test test = new Test();
        when(testServiceImp.put(test)).thenReturn(Optional.empty());
